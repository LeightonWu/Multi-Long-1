...
The difference between model 2 and 3 is significant. Model 3 fits the data better than model 2 given the significant chi statistic (`r chi23`). Allowing the slopes for KS2 scores to vary across schools enhances the model's predictive ability for GCSE scores, indicating that the relationship between KS2 and GCSE scores differs by school. However, as shown in Figure 3 and 4, the slopes exhibit minimal variation across schools.
...
The plot exhibits a fan-shaped pattern, where the variance of residuals increases with larger fitted values. indicating potential heteroscedasticity. Clusters of points on the right side indicate possible outliers or influential data points that may be impacting the model. Additionally, the residuals do not appear randomly distributed around zero, showing systematic bias and suggesting that the linearity assumption may be violated, as indicated by the nonlinear boundary in the extreme clusters.
...
The blue line illustrates a non-linear relationship between the residuals and the predictor (KS2 score), initially fluctuating around zero before sharply declining as the KS2 score increases. This indicates that the model fails to adequately capture the non-linear relationship, suggesting that the linear nature of the term may be inappropriate. Additionally, residuals become more negative at higher KS2 scores, indicating heteroscedasticity, and that model errors vary with the level of the KS2 score.
